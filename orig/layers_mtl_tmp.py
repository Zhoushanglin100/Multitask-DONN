    ...
    def generate_dsOp(self):
        ...
        if len(self.taskList) > 1:
            for task in self.taskList:
                self.dsOp[task] = torch.nn.ModuleList()

                if self.basicOp.in_channels != self.basicOp.out_channels or self.basicOp.stride != (1, 1):

                    self.dsOp[task].append(torch.nn.Parameter(torch.from_numpy(2 * np.pi * \
                                                                    torch.nn.init.xavier_uniform_(torch.empty(self.size, self.size)).numpy() ), 
                                            requires_grad=True))
    ...
    def generate_taskOp(self):
        ...
        if len(self.taskList) > 1:
            for task in self.taskList:
                ...
                self.policy[task] = torch.nn.Parameter(torch.tensor([0., 0., 0.]))  # Default initialization
        return
